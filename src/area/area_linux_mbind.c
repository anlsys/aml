...
{
	const aml_bitmap nodemask = data->nodemask;
	assert(data != NULL);
	...
	{
		for(int i = 0; i < AML_BITMAP_LEN; i++)
					   ...
					   void *ptr, size_t sz)
{
	...
	return mbind((void*)aligned, sz, data->policy, data->nodemask, AML_BITMAP_LEN, 0);
}
...
int aml_area_linux_mbind_setdata(struct aml_area_linux_mbind_data *data,
				 int policy, const aml_bitmap nodemask)
{
	assert(data != NULL);
	data->policy = policy;
...
int aml_area_linux_mbind_mempolicy_pre_bind(struct aml_area_linux_mbind_data *data)
{
	...
	int err;
	get_mempolicy(&policy, nodemask, AML_BITMAP_LEN, NULL, 0);
	err = set_mempolicy(data->policy, data->nodemask, AML_BITMAP_LEN);
