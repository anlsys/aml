                                                 ...
                                                 const size_t num)
{
	...
	if (parent->mapper->num_elements != NULL &&
		...
		head->array_size =
		        parent->mapper->num_elements[0](parent->host_copy);
                              ...
                              aml_dma_operator memcpy_host_dst)
{
	...
	err = aml_mapper_creator_create(out, c->stack->device_ptr,
                              ...
                              aml_dma_operator memcpy_host_dst)
{
	...
	err = aml_mapper_visitor_create(&visitor, src_ptr, mapper, dma_src_host,
	                                memcpy_src_host);
	if (err != AML_SUCCESS)
		return err;
	...
	err = -AML_ENOMEM;
	struct aml_mapper_creator *c =
	        AML_INNER_MALLOC_ARRAY(size, char, struct aml_mapper_creator);
	if (c == NULL)
		goto err_with_device_ptr;
	c->device_memory = dst_ptr;
	c->host_memory =
	        AML_INNER_MALLOC_GET_ARRAY(c, char, struct aml_mapper_creator);
	c->offset = 0;
