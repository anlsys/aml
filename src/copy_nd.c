	...
	if (d == 2)
		...
		for (int i = 0; i < elem_number[d - 1]; i++) {
			aml_copy_nd_helper(d - 1, dst, cumul_dst_pitch, src,
					   ...
					   elem_size);
			dst =
			    (void *)((uintptr_t) dst + cumul_dst_pitch[d - 2]);
			src =
			    (void *)((uintptr_t) src + cumul_src_pitch[d - 2]);
		}
	}
}

int aml_copy_nd(size_t d,
		void *dst,
		...
		const size_t * elem_number, size_t elem_size)
{
	...
	if (d == 1)
		...
		size_t *cumul_dst_pitch =
		    (size_t *) alloca((d - 1) * sizeof(size_t));
		...
		for (int i = 1; i < d - 1; i++) {
			...
			cumul_dst_pitch[i] =
			    dst_pitch[i] * cumul_dst_pitch[i - 1];
			cumul_src_pitch[i] =
			    src_pitch[i] * cumul_src_pitch[i - 1];
		}
		aml_copy_nd_helper(d, dst, cumul_dst_pitch, src,
				   cumul_src_pitch, elem_number, elem_size);
	}
	return 0;
}

static void aml_copy_ndstr_helper(size_t d,
				  void *dst,
	...
	if (d == 1)
		for (int i = 0; i < elem_number[0]; i++)
			memcpy((void *)((uintptr_t) dst +
					i * dst_stride[0] * elem_size),
			       (void *)((uintptr_t) src +
					i * src_stride[0] * elem_size),
			       elem_size);
	else {
		for (int i = 0; i < elem_number[d - 1]; i++) {
			...
			dst =
			    (void *)((uintptr_t) dst +
				     cumul_dst_pitch[d - 2] * dst_stride[d -
									 1]);
			src =
			    (void *)((uintptr_t) src +
				     cumul_src_pitch[d - 2] * src_stride[d -
									 1]);
		}
	}
}

int aml_copy_ndstr(size_t d,
		   void *dst,
		   const size_t * dst_pitch,
		   const size_t * dst_stride,
		   const void *src,
	...
	if (d == 1)
		...
		for (int i = 0; i < d; i++) {
			...
			if (target_dims[i] < d - 1)
				assert(dst_pitch[target_dims[i]] >=
				       elem_number[i]);
		...
		for (int i = 0; i < d - 1; i++) {
			assert(src_pitch[i] >= elem_number[i]);
			cumul_dst_pitch[i + 1] =
			    dst_pitch[i] * cumul_dst_pitch[i];
			cumul_src_pitch[i + 1] =
			    src_pitch[i] * cumul_src_pitch[i];
		}
